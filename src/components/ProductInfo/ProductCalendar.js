...
export class ProductCalendar extends Component {
    ...
    convertBorrowDateToListData = (value) =>{
        ...
        const consumer = this.state.orderDaysOfYear.filter(item => item.day === value.dayOfYear())
        let listData;
        if (this.state.orderDaysOfYear.some(item=>item.day === value.dayOfYear())) {
            listData = [
                {type: 'error', content: 'Product rented by:', consumerName: consumer[0]['consumerName'] , consumerAvatar: consumer[0]['consumerAvatar']},
                ];
            }
        return listData || [];
    }
    ...
    render(){
        return(
    <div style={{width: 1250, border: '1px solid #d9d9d9', borderRadius: 10, margin:"auto"}}>
                <Calendar dateCellRender={this.dateCellRender} monthCellRender={monthCellRender} />,
    </div>
