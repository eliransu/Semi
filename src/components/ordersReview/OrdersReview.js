...
class OrdersReview extends Component {
	...
	render() {
		const orders = orderStore.getAllOrdersAsConsumer ? orderStore.getAllOrdersAsConsumer : [];
		const noDataText = {
			filterConfirm: 'Ok',
			filterReset: 'Reset',
			emptyText: 'No Orders'
		...
		const columns = [
			{
				...
				key: 'product.name',
				render: (text, record) => (
					<NavLink style={{ textDecoration: 'underline' }} to={`/productPage/${record.product._id}`}>
						{text}
					</NavLink>
				)
			},
			...
			},
			{
				...
				render: (text, record) => <img src={`${record.product.images}`} height="42" width="42" />
			},
			{
				title: 'Provider',
		...
		return (
			<div>
				<Table dataSource={orders} columns={columns} locale={noDataText} />
				<AddReview />
			</div>
		);
	}
