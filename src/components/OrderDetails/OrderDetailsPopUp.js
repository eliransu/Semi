...
import { List, Button, InputNumber, Avatar, Tag, Divider, Col, Row } from 'antd';
...
class OrderDetailsPopUp extends Component {
	...
	redirectToConsumerStore = (consumer) => {
		this.props.closeModal();
		this.props.history.replace(`/user/${consumer.username}`);
	};
			...
			<div className="order-details" style={{ width: '400px' }}>
				...
				<Row>
						...
						<DescriptionItem
							...
							content={
								<React.Fragment>
									...
									<a onClick={() => this.redirectToConsumerStore(order.consumer)}>
					...
					<Col span={12}>
						<DescriptionItem
							title="Phone"
							content={order.consumer.phone_number ? order.consumer.phone_number : 'Not entered'}
						/>
					</Col>
					<Col span={12} />
				</Row>
				<Row>
					<Col span={12}>
						<DescriptionItem
							title="Country"
							content={order.consumer.address ? order.consumer.address.country : 'Not entered'}
						/>
					</Col>
					<Col span={12}>
						<DescriptionItem
							title="City"
							content={order.consumer.address ? order.consumer.address.city : 'Not entered'}
						/>
					</Col>
				</Row>
				<Row>
					<Col span={12}>
						<DescriptionItem
							title="Address"
							content={order.consumer.address ? order.consumer.address.street : 'Not entered'}
						/>
					</Col>
				</Row>
				<Divider />
				...
				<Row>
					<Col span={12}>
						<DescriptionItem title="Start Date" content={moment(order.start_time).format('DD-MM-YYYY')} />
					</Col>
					<Col span={12}>
						<DescriptionItem title="End Date" content={moment(order.finish_time).format('DD-MM-YYYY')} />
					</Col>
				</Row>
				<Row>
					<Col span={11} style={{ display: 'flex', flexWrap: 'nowrap', overflow: 'auto' }}>
						<DescriptionItem
							title="Product"
							content={<a href={`/productPage/${order.product._id}`}>{order.product.name}</a>}
						/>
					</Col>
					<Col span={12} style={{ paddingLeft: '15px' }}>
						<DescriptionItem
							title="Delivery"
							content={order.delivery ? <Tag color="green">yes</Tag> : <Tag color="red">no</Tag>} //user rent the product with delivery
						/>
					</Col>
				</Row>
				<Row>
					<Col span={24}>
						<DescriptionItem title="Renter Notes" content={order.remarks ? order.remarks : 'No notes'} />
					</Col>
				</Row>
				<Divider />
				<div
