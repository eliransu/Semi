...
class PaymentModalPage extends Component {
  ...
  toggleLoadingView = bool => {
    this.setState({ confirmLoading: bool });
  };
  handleOk = async e => {
    ...
    try {
      ...
      if (order) {
        ...
        AlertUtils.successAlert("Your Order Request sent to the Provider");
      }
    } catch (err) {
  ...
  render() {
    ...
    const creditCardNumber =
      isFieldTouched("creditCardNumber") && getFieldError("creditCardNumber");
...
const PaymentModalPageForm = Form.create({ name: "paymentModalForm" })(
  PaymentModalPage
);
